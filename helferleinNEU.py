...
from skimage.segmentation import mark_boundaries, watershed, relabel_sequential
...
def binmasks(elements, andor = 'and'):
    ...
    if andor is 'and': return reduce(np.bitwise_and, elements)
    if andor is 'or' : return reduce(np.bitwise_or, elements)
    else:
        print('andor should be either and/or not {}'.format(andor))
        return
#%%
def arrselect(arr, val = 0, bse = 'b'):
    if bse is 'b': return arr[arr>val]
    if bse is 's': return arr[arr<val]
    if bse is 'e': return arr[arr==val]
    else:
...
def shapefilter(lab, shape = 4, size = 3):
    from skimage.segmentation import relabel_sequential
    lab = sizefilter(lab, size)
    props = measure.regionprops(lab)
    imgs = [i.image for i in props]    
    rows = np.array([np.sum([np.count_nonzero(i)<shape for i in j])/j.shape[0] for j in imgs])
    cols = np.array([np.sum([np.count_nonzero(i)<shape for i in j.T])/j.shape[1] for j in imgs])
    out = np.bitwise_or(rows>.5, cols>.5)
    bla = np.append(True,out)[lab]
    lab[bla]=0
...
def readdict(path = None, name = 'info'):
    ...
    if not path:
        ...
        with open(path+name+'.csv', 'rb') as f:
            ...
            return dict(r)
#%%
def figsave(path, label = None, dpi = 300, plots_path = False):
...
def random_color(outformat = 'i'):
    ...
    return tuple(color)
# %%
def water_contur(image, val):
    contours = (find_contours(image, val))
    sor = np.argsort([i.size for i in contours])[::-1]
    
    water = np.zeros_like(image, dtype='uint16')
    wasize = water.size/2
    for i in range(20):
        out = np.zeros_like(image)
        out[contours[sor[i]][:,0].astype('int'), contours[sor[i]][:,1].astype('int')] = 1
        wats = (watershed(out)>1)
        if np.count_nonzero(wats==0)<wasize: 
            wats=np.invert(wats)
        water = np.where(wats,i+1,water)
